...
local cleuFunctions = {
  ["PALADIN"] = function(self, ...)
    ...
    if subEvent == "SPELL_HEAL" and sourceName == RaidCluster.playerName and spellID == 54968 and destName then
      ...
      if f and db then
        PixelGlow_Start(f, { db.glowColor.r, db.glowColor.g, db.glowColor.b, db.glowColor.a }, db.lines,
          db.frequency, db.length, db.thickness, db.xOffset, db.yOffset)
...
function RaidCluster:UpdateFrames(results, bool)
  ...
  for unit, count in pairs(results) do
    local frame, subGroup = self.currentframes[unit] and self.currentframes[unit].frame,
        self.currentframes[unit] and self.currentframes[unit].subGroup
...
function RaidCluster:specDetection()
  ...
  elseif Class == "PRIEST" then
    local hRaidFrame, hPixelGlow, dRaidFrame, dPixelGlow = db.holyPriestRaidFrameEnable,
        db.discoPriestRaidFrameEnable, db.holyPriestPixelGlowEnable, db.discoPriestPixelGlowEnable
